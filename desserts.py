...
link = []
link_img = []
url = 'https://calorizator.ru/recipes/category/desserts'
...
for i in range(1, 11):
    url = 'https://calorizator.ru/recipes/category/desserts?page='+str(i)
    page = requests.get(url)
    soup = BeautifulSoup(page.text, 'lxml')
    contents_name = soup.find_all('td', class_="views-field views-field-title active")
    contents_kcal = soup.find_all('td', class_="views-field views-field-field-kcal-value")
...
url = 'https://calorizator.ru/recipes/category/cakes'
page = requests.get(url)
soup = BeautifulSoup(page.text, 'lxml')
contents_name = soup.find_all('td', class_="views-field views-field-title active")
contents_kcal = soup.find_all('td', class_="views-field views-field-field-kcal-value")
...
for i in range(len(contents_name)):
    ...
    if contents_name[i].find('a') is not None:
        recipe.append(contents_name[i].text)
for i in range(1, 12):
    url = 'https://calorizator.ru/recipes/category/cakes?page='+str(i)
    page = requests.get(url)
    soup = BeautifulSoup(page.text, 'lxml')
    contents_name = soup.find_all('td', class_="views-field views-field-title active")
    contents_kcal = soup.find_all('td', class_="views-field views-field-field-kcal-value")
...
for i in range(len(link)):
    ...
    a = soup.find('span', itemprop='recipeYield').text
    if a == ' ':
        portions.append('?')
    else:
        portions.append(soup.find('span', itemprop='recipeYield').text)
...
for i in range(len(recipe)):
    ...
    cur.execute('INSERT INTO desserts VALUES( %s, %s, %s, %s, %s, %s, %s);', lip)
    connection.commit()
